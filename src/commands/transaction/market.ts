            ...
            try {
                ...
                if (confirmation.customId === 'confirm') {
                    try {
                        ...
                        await logToChannel(interaction.client, (transactionRecord.credit_used ? `💳 **${interaction.user.username}** used $${dollarize(transactionRecord.credit_used)} of ${config.theme.financialCompanyName} credit, bringing debt to $${dollarize(user.loan_balance + transactionRecord.credit_used)}.\n` : "") +
                            `🟢 **${interaction.user.username}** purchased ${quantity} share(s) of ${ticker} at $${dollarize(transactionRecord.price)} per share, bringing balance to $${dollarize(user.balance - (transactionRecord.balance_used || 0))}.`)
                    } catch(err) {
