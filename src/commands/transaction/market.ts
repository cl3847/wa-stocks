                ...
                if (confirmation.customId === 'confirm') {
                    try {
                        ...
                        await logToChannel(interaction.client,
                            `🔴 **${interaction.user.username}** sold ${quantity} share(s) of ${ticker} at $${dollarize(transactionRecord.price)} per share, increasing their balance to $${dollarize(user.balance + transactionRecord.price * transactionRecord.quantity)}.`)
                    } catch(err) {
